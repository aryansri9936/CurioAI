...
export default function FlashcardGame({ docId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      const r = await api.get(`/pdf/${docId}/chunks`).catch(() => null);
      const chunks =
        r?.data?.chunks
          ?.slice(0, 6)
          .map((c, i) => ({
            id: i,
            front: c.text.slice(0, 60) + '...',
            back: c.text.slice(60, 220),
          })) || [];
      setCards(chunks);
    }
  ...
  function pick(uid) {
    if (pairState.first === null) {
      setPairState(prev => ({ ...prev, first: uid }));
        ...
        <div className="space-x-2">
          <button
            ...
            className={`px-2 py-1 rounded ${
              mode === 'flash' ? 'bg-indigo-600 text-white' : ''
            }`}
          >
            Flashcards
          </button>
          <button
            ...
            className={`px-2 py-1 rounded ${
              mode === 'memory' ? 'bg-indigo-600 text-white' : ''
            }`}
          >
            Memory
          </button>
        ...
        <div className="mt-3">
          {cards.length === 0 && (
            <div className="text-gray-500">
              Upload a PDF to auto-generate cards
            </div>
          )}
          ...
          {cards.map(c => (
            <div key={c.id} className="p-2 border rounded mb-2">
              ...
              <div className="text-sm text-gray-600 mt-1">{c.back}</div>
            </div>
          ))}
        </div>
      )}
          ...
          {memoryDeck.map(card => (
            <div
              ...
              className={`p-3 border rounded text-center ${
                flipped[card.uid] ? 'bg-green-50' : 'bg-white'
              }`}
